...
def main() -> None:
    ...
    if not strategies or not horizons:
        raise ValueError("strategies and horizons must not be empty")
    ...
    if not df.empty and rank_col in df.columns:
        df = df.sort_values(by=[rank_col, "fusion_strategy", "target_horizon_days", "model"], ascending=True).reset_index(
            drop=True
        )
    ...
    with tempfile.TemporaryDirectory(prefix="train_compare_") as tmp_dir_raw:
        ...
        if best_by_rank:
            print(
                "best_by_rank_metric: "
                f"{best_by_rank.get('fusion_strategy')} h={best_by_rank.get('target_horizon_days')} "
                f"{best_by_rank.get('model')} ({rank_col}={best_by_rank.get(rank_col)})"
            )
        if best_by_mae:
            print(
                "best_by_mae_log: "
                f"{best_by_mae.get('fusion_strategy')} h={best_by_mae.get('target_horizon_days')} "
