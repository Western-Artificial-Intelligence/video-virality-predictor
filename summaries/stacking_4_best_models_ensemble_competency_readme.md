# Stacking Ensemble Competency README (Selected 4-Best-Model Set)
...
- `stacking/stacking_4_best_models_ensemble_competency.ipynb`
...
The residual report already has aligned per-video predictions and true labels for all required selected base models. That means we can evaluate ensemble competence immediately without rerunning:
...
## 3. Which Base Models Define the Selected 4-Model Stacking Set

From the checkpoint selection (see `virality-webapp/backend/app/constants.py` for the original source list), the selected 4-model stacking set is:
...
This is the same robust padded logic mirrored from backend range behavior.
...
- Horizon 7 was evaluated on `n=808` complete rows (all four selected bases present per `video_id`).
...
### 7.7 Robust range interval behavior (selected-4-model UI-style range)
...
1. Stacking over the selected 4-best-model set is genuinely more competent than any single base in this snapshot (OOF-evaluated).
...
For the question "How competent is stacking over the selected 4-best-model set?", this notebook is appropriate because it:
...
1. Open `stacking/stacking_4_best_models_ensemble_competency.ipynb`.
